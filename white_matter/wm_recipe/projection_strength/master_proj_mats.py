...
def make_regionalized_model(voxel_array, source_mask, target_mask, tree, vol, hemi,
    ...
    r_ids = [_r['id'] for _r in tree.get_structures_by_acronym(mpr.region_names)]
    if per_layer:
        s_key = source_mask.get_key(get_layer_specific_ids(tree, vol, mpr),
                                    hemisphere_id=2)
    else:
...
def layer_specific_matrix(mdl, tree, mpr, connection_type, layers):
    ...
    target_idx = numpy.array([target_names.index(_r) for _r in mpr.region_names])
    res = []
    w = []
    for l in layers:
        tmp_res = numpy.zeros((len(mpr.region_names), len(mpr.region_names)))
        tmp_w = numpy.zeros((len(mpr.region_names)))
        compound_src = [_r + l for _r in mpr.region_names]
...
def scale_to_target(cfg, mpr, Mtrx):
    if "scaling" in cfg:
        ...
        idx = mpr.region2idx(cfg["scaling"]["region"])
...
def master_proj_mats(cfg, mpr):
    ...
    for msrmnt in measurements:
        Mtrx[src_type + (('hemi', 'ipsi'), ('measurement', msrmnt))] = region_specific_matrix(mdl_i, tree, mpr, msrmnt)
        Mtrx[src_type + (('hemi', 'contra'), ('measurement', msrmnt))] = region_specific_matrix(mdl_c, tree, mpr, msrmnt)
    scale_to_target(cfg, mpr, Mtrx)
    return Mtrx
